...
const binyanei = ({ roots }: { roots: string[] }) => {
    ...
    return <div className={`w-screen min-h-screen text-black transition-colors ease-in-out duration-300 ${splashOut ? "bg-white" : "bg-black"}`}>

        <Transitional_ZoomIn delay={0} StartTransition={splashOut} className="flex flex-wrap justify-around items-center w-screen min-h-screen">
            {
                roots.map(root => {
...
const RootCard = ({ root, x, y }: { root: string, x: number, y: number }) => {
    ...
    return <motion.a
        ...
        href={`/${root}`}>
        <div className="w-full h-full py-2 px-4 md:px-12 md:py-8 hover:scale-150 transition ease-in-out duration-300 z-10 hover:z-0 font-normal font-mono">
            {root}
        </div>
    </motion.a>
}
